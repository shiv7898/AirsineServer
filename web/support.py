...
from app.schemas import SupportQueryCreate, SupportQueryResponse
...
def get_all_queries(request: Request, db: Session = Depends(get_db)):
    ...
    for q in queries:
        ...
        if not user:
            user = db.query(models.Distributor).filter(models.Distributor.id == q.user_id).first()
        if not user:
            user = db.query(models.AdminStaff).filter(models.AdminStaff.id == q.user_id).first()
        if not user:
            user = db.query(models.SuperAdmin).filter(models.SuperAdmin.id == q.user_id).first()
