...
from helpers.validators import to_numeric_or_string
...
def parse_therapy_data(text):
    ...
    if match:
        ...
        return data
    return {
        "usage_time": None, "compliance": None,
        "ahi_index": None, "avg_pressure": None, "leak_rate": None,
    }
...
def patient_section(request: Request, db: Session = Depends(get_db)):
    ...
    db_user = db.query(models.Patient).filter(models.Patient.id == user["user_id"]).first()
...
def get_profile(request: Request, db: Session = Depends(get_db)):
    ...
    if role == "distributor":
        db_user = db.query(models.Distributor).filter(models.Distributor.id == current_user["user_id"]).first()
    elif role == "doctor":
        db_user = db.query(models.Doctor).filter(models.Doctor.id == current_user["user_id"]).first()
    else:
        db_user = db.query(models.Patient).filter(models.Patient.id == current_user["user_id"]).first()
    ...
    return {
        ...
        "area": getattr(db_user, "area", None),
        "district": db_user.district,
        "state": db_user.state,
        "pincode": to_numeric_or_string(db_user.pincode),
        "hospital": getattr(db_user, "hospital", None),
        "specialisation": getattr(db_user, "specialisation", None),
        "qualification": getattr(db_user, "qualification", None),
        "experience": to_numeric_or_string(getattr(db_user, "experience", None)),
        "company_name": getattr(db_user, "company_name", None),
        "business_type": getattr(db_user, "business_type", None),
        "distributor_type": getattr(db_user, "distributor_type", None),
        "license_number": to_numeric_or_string(getattr(db_user, "license_number", None)),
...
def download_synced_pdf(days: int = 7, report_id: int = None, request: Request = None, db: Session = Depends(get_db)):
    ...
    if not report_data or not report_data.pdf_base64:
        raise HTTPException(status_code=404, detail="No synced PDF data found.")
...
def support_query(
    ...
    user_db = db.query(models.Patient).filter(models.Patient.id == current_user["user_id"]).first()
    if not user_db:
        raise HTTPException(status_code=404, detail="User not found")
    ...
    if images:
        for img in images:
            if img.filename:
                img_filename = f"pat_{current_user['user_id']}_{datetime.utcnow().strftime('%Y%m%d%H%M%S')}_{img.filename}"
