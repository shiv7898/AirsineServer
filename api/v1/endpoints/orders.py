...
def verify_referral(code: str, request: Request, db: Session = Depends(get_db)):
    ...
    referral_user = (
        db.query(models.Patient).filter(models.Patient.referral_code == code).first()
        or db.query(models.Doctor).filter(models.Doctor.referral_code == code).first()
        or db.query(models.Distributor).filter(models.Distributor.referral_code == code).first()
    )
