"""
api/v1/endpoints/auth.py  –  Mobile API authentication routes
"""
import random
...
def login(user: UserLogin, db: Session = Depends(get_db)):
    ...
    if not db_user:
        db_user = UserService.find_user_by_phone(db, user.email.strip())
    if not db_user:
        raise NotFoundException("User not found. Please check your email / phone number or register.")

    if db_user.role in ["super_admin", "admin", "sub_admin"]:
        raise AuthenticationException(
            "Access denied. Administrative accounts cannot log in to the mobile application."
        )
